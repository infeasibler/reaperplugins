...
local ok, input = reaper.GetUserInputs("Scenery - New scene", 1, "Length in bars:", tostring(cfg.default_bars))
if not ok then return end

local bars = math.floor(tonumber(input) or 0)
if bars < 1 then
    reaper.MB("Bar count must be a positive whole number.", "Scenery", 0)
    return
end
...
reaper.Undo_EndBlock2(0, "Scenery: New scene (" .. bars .. " bars)", -1)
reaper.PreventUIRefresh(-1)
