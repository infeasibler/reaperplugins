...
- Clone and Copy include items that overlap the source scene, even when they start before its region. By default they replace destination items inside the new scene while preserving portions that extend beyond it; audio lead-out overlaps are retained and crossfaded at scene and tile boundaries. "Skip occupied tracks when copying" is off by default and can be enabled to leave destination tracks containing items untouched.
