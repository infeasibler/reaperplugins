-- @noindex
-- Scenery: Toggle link with next scene
...
if not scene then
    reaper.MB("No active scene.", "Scenery", 0)
    return
end
if not scenes[scene.num + 1] then
    reaper.MB("No next scene to link with.", "Scenery", 0)
    return
end
...
reaper.Undo_EndBlock2(0, "Scenery: Toggle link with next scene", -1)
reaper.PreventUIRefresh(-1)
