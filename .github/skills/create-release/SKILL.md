...
3. Read commits after the current version's tag through `HEAD`. Use only changes relevant to the Scenery package and describe user-visible changes as concise, factual bullets. Do not include this release-preparation commit in the changelog. Stop and report if there are no release-worthy changes rather than publishing an empty release.
